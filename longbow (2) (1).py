...
def default_conf():
  ...
  for line in file:
    line = line.strip()
    changes = line.replace("ServerTokens Full", "ServerTokens Prod")
    replacement += changes + "\n"
...
def main():
  ...
  if system == 'Linux':
    ...
    proc=subprocess.Popen('\Apache24\\bin\httpd -v"', shell=True, stdout=subprocess.PIPE, )
    out=proc.communicate()[0]

    print("Hello you are running apache version:")    
    apache_version()

    VA = b'Server version: Apache/2.4.52 (Win64)\r\nApache Lounge VS16 Server built:   Dec 17 2021 10:17:38\r\n'
    ...
    if out == VA:
      ...
      if answer == 'y' or answer == 'Y':
        ...
        print("next step is optional asell it ")
      else:
